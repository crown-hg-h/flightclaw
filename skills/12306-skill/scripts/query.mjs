...
/** 12306 余票接口：/leftTicket/query 常返回 302 + JSON 体（status:false），真正数据在 c_url（如 leftTicket/queryG）。Node fetch 默认跟随重定向会落到拦截/错误页（HTML），必须 redirect: 'manual' 并自行解析 JSON、再请求 c_url。 */
async function fetchTicketJson(url, cookie, referer) {
  const res = await fetch(url, {
    headers: { ...HEADERS, Cookie: cookie, Referer: referer || HEADERS.Referer },
    redirect: 'manual',
  });
  ...
  if (!trimmed) {
    ...
    if (res.status >= 300 && res.status < 400 && (loc.includes('error.html') || loc.includes('mormhweb'))) {
      throw new Error(
        `12306 将余票查询重定向到错误页（${loc}），第二步接口可能被风控；请改用 12306 官网/App 或本机浏览器。`,
      );
    }
    throw new Error(`空响应 HTTP ${res.status}${loc ? ` Location: ${loc}` : ''}。`);
  }
  if (!trimmed.startsWith('{')) {
    const hint = trimmed.startsWith('<!DOCTYPE') || trimmed.startsWith('<html')
      ? '12306 返回了 HTML（常为风控/拦截页），非 JSON。'
      : '响应不是 JSON。';
    throw new Error(`${hint} HTTP ${res.status}。首段: ${trimmed.slice(0, 120)}`);
  }
  let json;
  try {
    json = JSON.parse(trimmed);
  } catch (e) {
    throw new Error(`解析 JSON 失败: ${e.message}；首段: ${trimmed.slice(0, 120)}`);
  }
  return { res, json };
}
...
async function queryTickets(from, to, travelDate) {
  ...
  const queryUrl = `https://kyfw.12306.cn/otn/leftTicket/query?${params}`;
  let { res, json } = await fetchTicketJson(queryUrl, cookie, HEADERS.Referer);

  if (json.data?.result) {
    return json.data;
  }

  // 新版：首包只告知真实路径 c_url（如 leftTicket/queryG），需再请求
  const cUrl = json.c_url;
  if (cUrl && typeof cUrl === 'string') {
    const secondUrl = `https://kyfw.12306.cn/otn/${cUrl}?${params}`;
    const second = await fetchTicketJson(secondUrl, cookie, 'https://kyfw.12306.cn/otn/leftTicket/query');
    json = second.json;
    if (json.data?.result) {
      return json.data;
    }
  ...
  console.error(
    'No ticket list in response. First status:',
    res.status,
    'Body:',
    JSON.stringify(json).slice(0, 800),
  );
  console.error(
    '\n提示：若仍无 data.result，多为 12306 对当前环境风控/接口变更。请用 12306 官网或 App 核对；或稍后重试。',
  );
  process.exit(1);
}
